...
SNAPSHOT_FILE = (
    Path(__file__).resolve().parent / "snapshots" / "snapshot_table_etp_financed.csv"
)
...
def test_etp_financed_pipeline(mock_get_structures):
    ...
    filtered_company_data = filter_company_data_by_structures(
        company_data, structures_filtered, 2023
    )
    montant, etp, total_etp, percentage = compute_etp_financed(
        filtered_company_data, structures_filtered, count_un_etp
    )
    result_table = get_etp_financed_table_by_categories(
        total_etp, etp, structures_filtered
    )
    result_table = result_table.sort_values(["category_type", "category"]).reset_index(
        drop=True
    )
    ...
    try:
        ...
        pd.testing.assert_frame_equal(
            result_table, expected_snapshot, check_dtype=False
        )
