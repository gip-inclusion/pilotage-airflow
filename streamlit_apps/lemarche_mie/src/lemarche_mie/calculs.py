...
def filter_structures_data(
    company_data: pandas.DataFrame, year_to_filter: int
) -> pandas.DataFrame:
    siren_list = [str(s) for s in company_data["SIREN"].unique()]
    marche_suivi_structure_df = get_marche_suivi_structure_df(
        siren_list, year_to_filter
    )
    marche_suivi_structure_df["siren"] = (
        marche_suivi_structure_df["structure_siret_actualise"].astype(str).str[:9]
    )
...
def get_found_sirens(
    ...
    db_sirens = (
        set(structures_filtered["siren"].astype(str))
        if not structures_filtered.empty
        else set()
    )
    ...
    if found_sirens:
        found_data = company_data_year[
            company_data_year["SIREN"].astype(str).isin(found_sirens)
        ]
...
def get_missing_sirens(
    ...
    if missing_sirens:
        missing_data = company_data_year[
            company_data_year["SIREN"].astype(str).isin(missing_sirens)
        ]
...
def add_cols_to_structures_data(
    structures_filtered: pandas.DataFrame,
) -> pandas.DataFrame:
    df = structures_filtered.copy()
    df["is_senior"] = (
        df["emi_sme_annee"].iloc[0] - df["salarie_annee_naissance"]
    ) >= 55
    df["is_junior"] = (df["emi_sme_annee"].iloc[0] - df["salarie_annee_naissance"]) < 26
    return df


def get_cleaned_structures_data(
    company_data: pandas.DataFrame, year_to_filter: int
) -> pandas.DataFrame:
    cleaned_structure = add_cols_to_structures_data(
        filter_structures_data(company_data, year_to_filter)
    )
    cleaned_structure[CATEGORY_TYPE_LIST] = (
        cleaned_structure[CATEGORY_TYPE_LIST].fillna("Inconnu").astype(str)
    )
...
def filter_company_data_by_structures(
    ...
    filtered_company_data = company_data[
        (
            company_data["SIREN"]
            .astype(str)
            .isin(structures_filtered["siren"].astype(str))
        )
...
def compute_etp_financed(
    filtered_company_data: pandas.DataFrame,
    structures_filtered: pandas.DataFrame,
    count_un_etp: int,
    ...
    percentage_etp_financed = (
        (etp_financed_by_company / total_etp_realised) * 100
        if total_etp_realised > 0
        else 0
    )
    return (
...
def compute_beneficiaries_financed_by_category(
    ...
    beneficiaires_etp = (
        structures_filtered.groupby("hash_nir")["nombre_etp_consommes_reels_annuels"]
        .sum()
        .reset_index()
    )

    beneficiaires_etp = beneficiaires_etp[
        beneficiaires_etp["nombre_etp_consommes_reels_annuels"] > 0
    ]
...
def merge_and_get_indicators_by_category(
    ...
    category_beneficiaries = compute_beneficiaries_financed_by_category(
        structures_filtered, category_type
    )
...
def get_etp_financed_table_by_categories(
    ...
    return etp_financed_by_categories[
        ["category_type", "category", "Nombre ETP", "Ratio ETP", "Nombre beneficiaires"]
    ]
