...
from streamlitApps.lemarche_mesure_impact_entreprise.calculs import (
...
MOCK_STRUCTURES = pd.DataFrame({
    "structure_siret_signature": ["12345678900000","98765432100000"],
    "emi_sme_annee": [2023,2023],
    "nombre_etp_consommes_reels_annuels":[1.6,2.4],
    "identifiant_salarie": ["identifiant_1", "identifiant2"],
...
@patch("streamlitApps.lemarche_mesure_impact_entreprise.calculs.get_marche_suivi_structure_df")
def test_filter_structures_data(mock_get_structures, company_data, year_to_filter, mock_df, expected_df):
    ...
    result_sorted = result.sort_values("structure_siret_signature").reset_index(drop=True)
    expected_sorted = expected_df.sort_values("structure_siret_signature").reset_index(drop=True)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (COMPANY_BASE, pd.DataFrame({"nombre_etp_consommes_reels_annuels":[0]}), 300000, EXPECTED_ETP_FINANCED_BY_COMPANY, 0,0.0)
    ]
)

def test_compute_etp_financed(company_data, structures_data,expected_montant, expected_etp, expected_total_etp, expected_percentage):
...
def test_compute_etp_financed_by_category():
    ...
    assert pytest.approx(
        result["perc_etp_financed_by_company_per_category"].sum()
    ) == 1.0
...
def test_compute_beneficiaries_financed_by_category():
    df = pd.DataFrame({
        "adresse_qpv": ["True", "False", "False", "False", "True"],
        "identifiant_salarie":["id_1", "id_2","id_2","id_3","id_1"]
        })

    result = compute_beneficiaries_financed_by_category(
        structures_filtered=df,
        category_type="adresse_qpv"
