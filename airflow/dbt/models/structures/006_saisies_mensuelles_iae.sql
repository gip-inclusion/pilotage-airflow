    ...
    select distinct
        ...
        case
            when
                trim(
                    substr(
                        trim(salarie.salarie_codeinseecom),
                        1,
                        char_length(salarie.salarie_codeinseecom) - 2
                    )
                ) like ('%97%')
                then
                    trim(
            ...
            when
                ...
                trim(
                    substr(
                        trim(salarie.salarie_codeinseecom),
                        1,
                        char_length(salarie.salarie_codeinseecom) - 3
                    )
                )
...
salarie as (
    select
        ...
        case
            when
                (annee_en_cours - salarie.salarie_annee_naissance) <= 25
                then 'a- Moins de 26 ans'
            when
                (annee_en_cours - salarie.salarie_annee_naissance) >= 26
                and (annee_en_cours - salarie.salarie_annee_naissance) <= 30
                then 'b- Entre 26 ans et 30 ans'
            when
                (annee_en_cours - salarie.salarie_annee_naissance) >= 31
                and (annee_en_cours - salarie.salarie_annee_naissance) <= 50
                then 'c- Entre 31 ans et 50 ans'
            when
                (annee_en_cours - salarie.salarie_annee_naissance) >= 51
                then 'd- 51 ans et plus'
            else 'autre'
        end as tranche_age
    from constantes
    cross join
...
select distinct
    ...
    infra.nom_epci,
    make_date(
        cast(emi.emi_sme_annee as integer), cast(emi.emi_sme_mois as integer), 1
    ) as date_saisie,
    to_date(
        emi.emi_date_validation, 'dd/mm/yyyy'
    ) as date_validation_declaration,
    case
    ...
    concat(
        ctr_mis.contrat_code_rome, '-', code_rome.description_code_rome
    ) as metier,
    case
        when
            niv_formation.rnf_libelle_niveau_form_empl = 'JAMAIS SCOLARISE'
            then 'a- Jamais scolarisé'
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'PAS DE FORMATION AU DELA DE LA SCOLARITE OBLIG.'
            then 'b- Pas de formation au delà de la scolarité obligatoire'
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'PERSONNES AVEC QUALIFICATIONS NON CERTIFIANTES'
            then 'c- Personnes avec qualifications non certifiantes'
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'FORMATION COURTE D''UNE DUREE D''UN AN'
            then 'd- Formation courte d''une durée d''un an'
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'FORMATION DE NIVEAU CAP OU BEP'
            then 'e- Formation de niveau CAP ou BEP'
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'DIPLÔME OBTENU CAP OU BEP'
            then 'f- Diplôme obtenu CAP ou BEP'
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'BREVET DE TECHNICIEN OU BACCALAUREAT PROFESSIONNEL'
        ...
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'FORMATION DE NIVEAU BTS OU DUT'
        ...
        when
            niv_formation.rnf_libelle_niveau_form_empl
            = 'TROISIEME CYCLE OU ECOLE D''INGENIEUR'
            then 'k- Troisièmes cycle ou école d''ingénieur'
        else 'l- Autre'
    end as niveau_formation_salarie,
    to_date(emi.emi_date_fin_reelle, 'dd/mm/yyyy'
    ) as date_sortie,
    case
...
left join "fluxIAE_AnnexeFinanciere_v2" as af
    on
        ...
        and af_etat_annexe_financiere_code in ('VALIDE', 'PROVISOIRE', 'CLOTURE'
        )
        and emi.emi_sme_annee >= annee_en_cours_2
...
left join
    sa_ept as ept
    on ept.code_comm = structure.structure_adresse_admin_code_insee
left join
    sa_zones_infradepartementales as infra
    on infra.code_commune = structure.structure_adresse_admin_code_insee
