drop table if exists igas_etp_conventionnes_par_eiti_par_annee;
create table igas_etp_conventionnes_par_eiti_par_annee as (
    select distinct
        ...
        sum((af.af_etp_postes_insertion * ((date_part('year', af_date_fin_effet_v2) - date_part('year', af_date_debut_effet_v2)) * 12 + (date_part('month', af_date_fin_effet_v2) - date_part('month', af_date_debut_effet_v2)) + 1) / 12)) as effectif_annuel_conv
    from
