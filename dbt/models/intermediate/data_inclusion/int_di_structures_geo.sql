with structures as (
    select
        {{ dbt_utils.star(ref('stg_di_structures')) }}
    from {{ ref('stg_di_structures') }}
),

deduplicated as (
    select id
    from {{ ref('stg_di_structures_deduplicated') }}
),
...
select
    structures.*,
    deduplicated.id is not null                                                  as is_not_duplicate,
...
from structures
left join geo on structures.code_insee = geo.code_commune_insee
left join geo_by_postal on structures.code_postal = geo_by_postal.code_postal_principal
left join deduplicated on structures.id = deduplicated.id
