...
prestations as (
    ...
    select
        ...
        prestation.prestation_key_raw as prestation_key,

        prestation.prestation_value   as prestation_json,

        regexp_replace(
            regexp_replace(prestation.prestation_key_raw, '-[0-9]+$', ''),
            '-+$',
            ''
        )                             as prestation_key_base_raw

    from source
    cross join lateral jsonb_each(
        coalesce(nullif(source.prestations_json, ''), '{}')::jsonb
    ) as prestation (prestation_key_raw, prestation_value)
...
prestations_enriched as (
    ...
    select
        ...
        prestation_map.prestation_group,

        prestation_map.prestation_label,
        prestation_map.orp_status,
        coalesce(
            prestation_map.prestation_key_base,
            prestations.prestation_key_base_raw
        )                                           as prestation_key_base,
        coalesce(prestation_map.is_reliable, false) as is_reliable_prestation_mapping,
        prestation_map.prestation_key_base is null  as is_unmapped_prestation

    from prestations

    left join prestation_mapping as prestation_map
        on
            prestations.prestation_key = prestation_map.prestation_key_base
            or prestations.prestation_key like prestation_map.prestation_key_base || '-%'
