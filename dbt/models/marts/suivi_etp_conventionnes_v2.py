def computing_etp_part(df):
    df["Montant_total_aide"] = (
        df["effectif_mensuel_conventionne"] * df["af_montant_unitaire_annuel_valeur"] / 12 * df["duree_annexe"]
    )
    ...
    df["etp_conventionnes_cd"] = (df["effectif_mensuel_conventionne"] * df["part_conventionnement_cd"]) * (
        df["duree_annexe"] / 12
    )
    # ETP conventionnés Etat
    df["etp_conventionnes_etat"] = (df["effectif_mensuel_conventionne"] * (1 - df["part_conventionnement_cd"])) * (
        df["duree_annexe"] / 12
    )
    return df
