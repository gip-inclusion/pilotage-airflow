...
left join {{ ref('regions') }} as regions
    on regions."REG" = communes.code_region
left join {{ ref('departements') }} as departements
    on departements."DEP" = communes.code_dept
left join {{ ref('epci') }} as epci
    on epci."EPCI" = communes.code_epci
left join {{ ref('epci_libelle') }} as epci_libelle
    on epci."NATURE_EPCI" = epci_libelle.code_type_epci
left join {{ ref('arrondissements') }} as arrondissements
    on trim('0' from arrondissements."ARR") = communes.code_arrondissement
left join {{ ref('zone_emploi') }} as zone_emploi
