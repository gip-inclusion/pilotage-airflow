select
    {{ pilo_star(ref('etat_mensuel_sorties'), relation_alias='ems') }},
    {{ pilo_star(ref('motif_sorties'), relation_alias='sorties') }},
...
from {{ ref("etat_mensuel_sorties") }} as ems
left join {{ ref("motif_sorties") }} as sorties
    on
...
where
    ...
    and ems.af_mesure_dispositif_code in ('EITI_DC')
