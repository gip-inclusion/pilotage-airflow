select
    ...
    (array_agg(
        ...
        order by ctr.contrat_date_embauche desc
    ))[1]                                    as region,
    (array_agg(
    ...
    (array_agg(
        ctr.motif_sortie
        order by ctr.contrat_date_embauche desc
    ))[1]                                    as motif_sortie_dernier_contrat,
    (array_agg(
        ctr.categorie_sortie
        order by ctr.contrat_date_embauche desc
    ))[1]                                    as categorie_sortie_dernier_contrat,
    (array_agg(
        ctr.contrat_date_sortie_definitive
        order by ctr.contrat_date_embauche desc
    ))[1]                                    as date_sortie_definitive_dernier_contrat,
    (array_agg(
        ctr.salarie_sorti
        order by ctr.contrat_date_embauche desc
    ))[1]                                    as salarie_sortie_dernier_contrat,
    case
