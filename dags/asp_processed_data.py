    ...
    **default_dag_args(),
    ...
    def store_asp(**kwargs):
        ...
        with ftputil.FTPHost(*ftp.bucket_connection()) as host_ft:
            ...
            with host_ft.open(f"donnees_asp/{asp_file}", "rb") as asp_data_file:
                # Pass binary content directly to pandas using BytesIO
                asp_data = pd.read_excel(BytesIO(asp_data_file.read()))

                asp_data = asp_data.rename(
                    columns={
                ...
                asp_data["numero_annexe_financiere"] = asp_data["0 - Numéro de l'annexe / avenant"].apply(
                    lambda x: re.sub(r"\s+|A\d.*$", "", x)
                )

        asp_data.to_sql("fluxIAE_BenefSorties_ASP", con=db.connection_engine(), if_exists="replace", index=False)
