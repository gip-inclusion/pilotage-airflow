...
with airflow.DAG(
    ...
    params={"refresh_oneshot": Param(False, type="boolean")},
    **dag_args,
) as dag:
    ...
    def params_check(params=None, **kwargs):
        is_refresh_oneshot = params.get("refresh_oneshot")
        if is_refresh_oneshot:
            kwargs["ti"].xcom_push("dbt_seed_args", "--full-refresh")
            kwargs["ti"].xcom_push("dbt_run_args", "--select marts.oneshot")
        else:
            kwargs["ti"].xcom_push("dbt_seed_args", "")
            kwargs["ti"].xcom_push("dbt_run_args", "--select candidatures_ddets50")
