...
def get_file_content(df, name):
    logger.info("Generate file %r", name)
    buffer = io.StringIO()
    # Write the header. FIXME: Make the reference month not hardcoded
    buffer.write(f"000|{name}|INCLUSION|MESUREIMPACT|{datetime.date.today():%d%m%Y}|01052025|{len(df)}\n")
    ...
    **dag_args,
    ...
    def process(*, params):
        ...
        with py7zr.SevenZipFile(archive_buffer, "w") as archive:
            archive.writestr(get_file_content(processed_df, file_name), file_name)
