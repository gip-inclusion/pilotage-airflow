...
with airflow.DAG(
    ...
    **dag_args,
    ...
    def params_check(params=None, **kwargs):
        ...
        if is_full_refresh:
            kwargs["ti"].xcom_push("dbt_run_args", "--exclude marts.weekly stg.weekly")
            kwargs["ti"].xcom_push("dbt_seed_args", "--full-refresh")
        else:
            kwargs["ti"].xcom_push("dbt_run_args", "--exclude marts.weekly stg.weekly legacy.oneshot marts.oneshot")
